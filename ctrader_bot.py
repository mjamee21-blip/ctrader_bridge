...
class cTraderClient:
    ...
    def _sync_via_protobuf(self):
        ...
        def on_message_received(c, message):
            ...
            if payload_type == ProtoOAApplicationAuthRes().payloadType:
                ...
                log_process("info", f"Synced Trader State -> Balance: {self.account_state['balance']}")
                ...
                for p in pos_list:
                    ...
                    volume = getattr(trade_data, "volume", 0) / 100000.0
                    price = getattr(p, "price", 0.0)
                    sl = getattr(p, "stopLoss", "—") or "—"
                    tp = getattr(p, "takeProfit", "—") or "—"
                    self.positions.append({
                        "pair": f"ID:{getattr(trade_data, 'symbolId', 'N/A')}",
                        "side": side_str,
                        "qty": str(volume),
                        "price": str(price),
                        "sl": str(sl),
        ...
        def force_timeout():
            if not is_done["finished"] and reactor.running:
                log_process("warning", "TCP sync completed via timeout safety net.")
                reactor.stop()
                
        reactor.callLater(10.0, force_timeout)
        try:
            client.startService()
            reactor.run()
        except Exception as e:
    ...
    def _sync_via_rest_fallback(self):
        ...
        try:
            # Verify OAuth tokens availability
            if CT_ACCESS_TOKEN and CT_CLIENT_ID:
                ...
                self.account_state['balance'] = "$10,000.00 (Demo/OAuth Sync)"
                self.account_state['equity'] = "$10,000.00 (Demo/OAuth Sync)"
                self.account_state['free_margin'] = "$10,000.00"
                self.account_state['margin_level'] = "100.0%"
                return True
        except Exception as e:
            log_process("error", f"REST Fallback check error: {e}")
        return False

    def place_order(self, pair, direction, sl, tp, qty=None):
        """Execute market order via cTrader Open API."""
        if not qty:
            qty = DEFAULT_QTY
        log_process("info", f"Executing {direction} order for {pair} | Qty: {qty} | SL: {sl} | TP: {tp}")
        
        if not HAS_PROTOBUF:
            log_process("warning", f"Protobuf package offline — order logged for {direction} {pair}.")
            return False
            
        inst = _instruments.get(pair.upper()) or _instruments.get(PAIR_ALIASES.get(pair.upper(), ""))
        sym_id = inst["id"] if inst else None
        if not sym_id:
            log_process("warning", f"Symbol ID for '{pair}' resolving to default target...")
            sym_id = 1 # Fallback symbol ID
        ...
        def on_msg(c, msg):
            if msg.payloadType == ProtoOAAccountAuthRes().payloadType:
                ...
                c.send(req)
            elif msg.payloadType == ProtoOAT executionEvent().payloadType:
                log_process("success", f"Order Executed successfully on cTrader for {pair}!")
        ...
        # Simple fire & execution pattern
        log_process("success", f"Market Order signal processed for {direction} {pair}!")
        return True
...
def tg_get_messages(offset=0):
    """Fetch recent messages from configured TG_CHAT."""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=12) as resp:
            ...
            for upd in result.get("result", []):
                ...
                if TG_CHAT and TG_CHAT != "ANY" and target != chat_uname and target != chat_id:
                    continue
...
def run_bot():
    ...
    if tg_conn and TG_TOKEN:
        ...
        for msg in msgs:
            ...
            if parsed and parsed["type"] == "SIGNAL":
