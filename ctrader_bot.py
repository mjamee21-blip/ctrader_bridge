#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# cTrader Telegram Bot + Dashboard (Fully Enhanced)
# ALL ORIGINAL FEATURES PRESERVED + ADDITIONAL HELPFUL FEATURES
# 
# Features:
#   ✅ Connection status for cTrader AND Telegram
...
#   ✅ Shows SL/TP and P&L results for each trade from live cTrader data
...
# Common pair aliases — maps signal names to cTrader instrument names
PAIR_ALIASES = {
...
CT_BASE = "https://live.ctrader.com" if CT_ENV.lower() == "live" else "https://demo.ctrader.com"
_last_update_id = 0
_instruments = {}
...
class cTraderClient:
    ...
    def load_instruments(self):
        """
        Load ALL instruments from cTrader and build a name→ID mapping.
        """
        global _instruments
        _instruments = {}
...
# =====================================================================
# FLEXIBLE FIELD RESOLVER FOR cTrader API
# =====================================================================
...
def generate_dashboard_html(client, tl_connected, tl_error, tg_connected, tg_info):
    ...
    for pos in positions:
        if not isinstance(pos, dict):
            continue
        try:
            # Log first position parsed
            if len(positions_data) == 0:
                log_process("info", f"Parsed first position: {pos.get('pair')} {pos.get('side')} qty={pos.get('qty')} @ {pos.get('price')}")
    ...
    # --- Health indicators ---
    health_checks = {
        "tl_auth": {"ok": tl_connected, "label": "cTrader Auth", "icon": "🔐"},
...
<td>{log['message']}</td>
    ...
    gh_workflow_default = GH_WORKFLOW if GH_WORKFLOW else "ctrader.yml"
...
<body>
    ...
    <div id="settingsModal" class="modal">
        <div class="modal-content">
            ...
            <input type="text" id="gh_repo" placeholder="ctrader_bridge">
            <label>Workflow Filename</label>
            <input type="text" id="gh_workflow" value="ctrader.yml">
...
def run_bot():
    """Run the trading bot: fetch Telegram signals → place cTrader orders."""
    try:
        save_heartbeat("bot", "running", "Initializing bot...")
        log_process("info", "=== BOT CYCLE STARTED ===")
        
        if not (CT_CLIENT_ID and CT_ACCESS_TOKEN and TG_TOKEN and TG_CHAT):
            log_process("error", "Missing required secrets (CT_CLIENT_ID, CT_ACCESS_TOKEN, TG_TOKEN, TG_CHAT)")
...
def generate_dashboard():
    ...
    try:
        ...
        if not tl_connected:
            ...
            log_process("warning", f"cTrader: DISCONNECTED — {tl_error}")
        else:
...
<html><head><meta charset="utf-8"><title>cTrader Dashboard</title></head>
