...
class cTraderClient:
    ...
    def place_order(self, pair, direction, sl, tp, qty=None):
        ...
        if not instrument:
            log_process("error", f"Cannot find instrument: {pair}")
            return False

        log_process("info", f"ORDER: {direction} {pair} Qty:{qty} SL:{sl} TP:{tp}")
        log_process("success", f"Order signal processed for {pair}")
        return True

    def close_position(self, pos_id):
        log_process("info", f"Close position: {pos_id}")
        return True

    def modify_position(self, pos_id, new_sl):
        log_process("info", f"Modify position {pos_id} SL:{new_sl}")
        return True
        ...
        if TG_TOKEN:
            ...
            for msg in messages:
                ...
                if parsed["type"] == "SIGNAL":
                    client.place_order(pair=parsed["pair"], direction=parsed["direction"], sl=parsed["sl"], tp=parsed["tp"])
                elif parsed["type"] == "TPSL_HIT":
                    log_process("info", f"TP/SL hit on {parsed['pair']}")
                    positions = client.get_open_positions()
                    for pos in positions:
                        if isinstance(pos, dict) and pos.get("id"):
                            client.close_position(pos["id"])
                elif parsed["type"] == "SL_UPDATE":
                    log_process("info", f"SL update {parsed['pair']} → {parsed['new_sl']}")
                    positions = client.get_open_positions()
                    for pos in positions:
                        if isinstance(pos, dict) and pos.get("id"):
                            client.modify_position(pos["id"], parsed["new_sl"])

            log_process("info", f"=== BOT CYCLE COMPLETE === ({signal_count} signals)")
            save_heartbeat("bot", "completed", f"Processed {signal_count} signals")
        else:
            log_process("info", "Telegram not configured")
            save_heartbeat("bot", "completed", "No Telegram")
