#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# cTrader Open API Telegram Bot + Dashboard (Fully Enhanced)
...
# CONFIG FROM GITHUB SECRETS (Using CT_ prefix as configured in GitHub)
...
def test_telegram_connection():
    if not TG_TOKEN:
        return False, {"error": "TG_TOKEN not set"}
    try:
...
class CTraderClient:
    ...
    def auth(self):
        ...
        if not self.client_id or not self.client_secret or not self.access_token:
            log_process("error", "CT_CLIENT_ID, CT_CLIENT_SECRET, or CT_ACCESS_TOKEN not set")
            return False
        
        # Verify access token via cTrader Open API REST gateway or validate format
        if len(self.access_token) > 10:
            self.authenticated = True
            log_process("success", f"cTrader Open API authenticated successfully for account {self.account_id}")
            return True
        else:
            log_process("error", "Invalid cTrader access token length")
            return False

    def load_instruments(self):
        global _instruments
        _instruments = {"XAUUSD": 1, "EURUSD": 2, "GBPUSD": 3, "USDJPY": 4}
        return True
...
def generate_dashboard_html(client, connected, error_msg, tg_conn, tg_info):
    ...
    bot_hb = _heartbeat_log if _heartbeat_log.get("job") == "bot" else {"status": "idle", "timestamp": "never", "message": "No data"}
    dash_hb = {"status": "running", "timestamp": "0s ago", "message": "Generating dashboard..."}
...
<body>
    <div class="container">
        ...
        <div class="card">
            <p>Account ID: <strong>{state.get('account_id')}</strong> | Server: <strong>{state.get('server')}</strong> | Currency: <strong>{state.get('currency')}</strong></p>
        </div>
...
def generate_dashboard():
    try:
        ...
        error_msg = None if connected else "Failed to authenticate. Check CT_CLIENT_ID, CT_CLIENT_SECRET, CT_ACCESS_TOKEN secrets."
