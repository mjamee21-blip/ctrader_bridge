#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# TradeLocker Telegram Bot + Dashboard (Enhanced)
# Features:
...
class TradeLockerClient:
    ...
    def find_instrument(self, pair_name):
        ...
        # 5. Not found — print available instruments for debugging
    ...
    def place_order(self, pair, direction, sl, tp, qty=None):
        ...
        # Step 1: Resolve pair name → numeric instrument ID
        instrument = self.find_instrument(pair)
        if not instrument:
            # Try reloading instruments once
        ...
        # Step 2: Build MARKET order payload (no price — market execution)
        payload = {
        ...
        if result.get("error") or result.get("s") == "error":
            ...
            # Fallback: If route is forbidden, try a limit order near current price
...
def parse_signal(text):
    ...
    # Extract SL and TP from all lines — IGNORE any entry/REF price
    for line in lines:
        cl = re.sub(r"<[^>]+>", "", line).strip()
        # SL: matches "SL:" or "SL " followed by a number
        ...
        if m and sl is None:
            try:
                ...
                pass
        # TP: matches "TP:" or "TP " followed by a number
...
def generate_dashboard_html(client, tl_connected, tl_error, tg_connected, tg_info):
    """Generate the full dashboard HTML with connection status, fetch button, SL/TP, and results."""
...
<td class="time">{o['time']}</td>
    ...
    # --- JavaScript for "Fetch Now" button ---
    js_code = """
<script>
function getGH() {
    return {
...
function fetchNow() {
    ...
    if (!s.token || !s.owner || !s.repo) {
        alert('Please configure GitHub settings first.\\nClick the ⚙️ button to set your GitHub token and repo info.');
        openSettings();
        return;
    }
...
def run_bot():
    ...
    # Load instruments (name → numeric ID mapping)
    client.load_instruments()

    # Fetch and process Telegram messages
    ...
    for msg in messages:
        ...
        if parsed["type"] == "SIGNAL":
            # Place MARKET order immediately (ignores signal price/REF)
            client.place_order(
                pair=parsed["pair"],
            ...
            # Close position when TP or SL is hit
            pair = parsed["pair"]
            ...
            # Adjust SL on existing position
            pair = parsed["pair"]
...
def generate_dashboard():
    ...
    # Generate HTML (always — even on error, so the dashboard shows status)
...
if __name__ == "__main__":
    try:
        ...
        # On fatal error in dashboard mode, write an error page
        if MODE == "dashboard":
            try:
                os.makedirs("docs", exist_ok=True)
