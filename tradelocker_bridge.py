...
#
# FIX-24: AUTO-GENERATE requirements.txt ON STARTUP.
#         GitHub Actions / CI pipelines require a requirements.txt or
#         pyproject.toml to be present. Since this project uses only the
#         Python standard library, we write a minimal requirements.txt
#         (with a comment explaining it) at startup so CI never fails
#         with "No file matched to **/requirements.txt".
#
# =====================================================
...
# =====================================================
# FIX-24: AUTO-GENERATE requirements.txt
# GitHub Actions looks for requirements.txt or pyproject.toml.
# This project uses only the Python standard library, so we write
# a minimal requirements.txt on every startup to satisfy CI checks.
# =====================================================

def _ensure_requirements_txt():
    """
    Write a requirements.txt into the same directory as this script if one
    does not already exist (or if it is empty).  The file documents that
    this project has NO third-party dependencies — it uses only the Python
    standard library — while satisfying GitHub Actions / pip-install steps
    that require the file to be present.
    """
    req_path = os.path.join(SCRIPT_DIR, "requirements.txt")
    content = (
        "# This project uses only the Python standard library.\n"
        "# No third-party packages are required.\n"
        "# This file is auto-generated by tradelocker_bridge.py (FIX-24)\n"
        "# to satisfy GitHub Actions pip dependency checks.\n"
    )
    try:
        ...
        # Non-fatal: log file write may fail in read-only environments
        pass
...
class TradeLockerClient:
    ...
    def _track_sltp_status(self, positions):
        ...
        if check_result["missing_both"]:
            log("WARN", f"  {len(check_result['missing_both'])} open position(s) "
                f"missing BOTH SL and TP: {', '.join(check_result['missing_both'][:5])}")
        elif check_result["missing_sl"]:
            log("WARN", f"  {len(check_result['missing_sl'])} open position(s) "
                f"missing SL: {', '.join(check_result['missing_sl'][:5])}")
        elif check_result["missing_tp"]:
            log("WARN", f"  {len(check_result['missing_tp'])} open position(s) "
...
def run_bridge_cycle(tl_client):
    ...
    # FIX-21: CACHING to reduce API load — only refresh account info every 30s
            ...
            if history:
                ...
                if abs(history_count - _last_history_order_count) > 10 or _cache_trade_history_time == 0:
                    ...
                    if results["closed_trades"] > prev_closed:
                        ...
                        log("INFO", f"New closed trade | {newest.get('pair')} "
  ...
  <div class="card">
    ...
    if sltp_check.get("missing_both") or sltp_check.get("missing_sl") or sltp_check.get("missing_tp"):
        ...
        print(f'<div class="err-banner" style="background:rgba(248,{81 if warn_level=="red" else 160},73,.08);border-color:var(--{warn_level})">'
              f'SL/TP MISSING: {" | ".join(msg_parts)}</div>')
    ...
    if not positions:
        ...
        if positions and isinstance(positions[0], dict):
            ...
            print(f'<tr><td colspan="11" style="font-size:9px;color:var(--text2);'
                  f'padding:4px 8px">Raw field names from API: {html.escape(raw_keys)}</td></tr>')
